...
test.describe("subtitle preview scaling", () => {
  test("scales subtitle measurements proportionally to container height", () => {
    ...
    expect(scaleSubtitlePx(9, compactHeight)).toBe(8);
    expect(scaleSubtitlePx(12, compactHeight)).toBe(8);
    expect(scaleSubtitlePx(24, compactHeight)).toBe(8);
  });

  test("supports an explicit minimum size", () => {
    expect(scaleSubtitlePx(50, 500, 20)).toBe(20);
    expect(scaleSubtitlePx(50, 500, 0)).toBeCloseTo(13.02, 2);
  });
