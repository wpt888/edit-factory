...
test("clip inspector shows the segment transform panel", async ({ page }) => {
  const editor = await openFullEditor(page);

  await editor.getByTestId("composition-clip-body-a").click();

  const panel = page.getByTestId("clip-transform-panel");
  await expect(panel).toBeVisible();
  await expect(panel.getByText("Transforms")).toBeVisible();
  await expect(panel.getByText("Blur fill")).toBeVisible();
  await expect(panel.getByText("Saturation")).toBeVisible();

  await page.screenshot({ path: "screenshots/clip-transform-panel.png", fullPage: true });
});
