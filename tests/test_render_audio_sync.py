...
def test_render_uses_edited_video_timeline_as_output_clock(tmp_path, monkeypatch):
    from app.api import library_routes
    ...
    monkeypatch.setattr(
        library_routes,
        "safe_ffmpeg_run",
        lambda *_args, **_kwargs: SimpleNamespace(returncode=0, stdout="5.0\n", stderr=""),
    )
    ...
    asyncio.run(
        library_routes._render_with_preset(
            ...
            srt_path=None,
            subtitle_settings=None,
            preset={"name": "Preview", "width": 540, "height": 960, "fps": 30},
            output_path=output_path,
    ...
    command = captured_commands[0]
    duration_index = command.index("-t")
    assert float(command[duration_index + 1]) == pytest.approx(7.25)
