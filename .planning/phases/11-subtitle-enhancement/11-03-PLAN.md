...
3. **Integrate into library page** (`frontend/src/app/library/page.tsx`):
   ...
   NOTE: The subtitle settings state is `editingSubtitleSettings` / `setEditingSubtitleSettings` (line 331, local useState). This is NOT the useSubtitleSettings hook — library page uses a local state variable.

   c. Add form data submission for subtitle enhancement params in the render function (after the existing Phase 9 filter formData.appends at lines 854-858):
   ```typescript
   // Subtitle enhancement (Phase 11)
   ...
   NOTE: The subtitle settings state variable is `editingSubtitleSettings` (line 331, local useState, NOT useSubtitleSettings hook). The setter is `setEditingSubtitleSettings`. The hook `useSubtitleSettings` is NOT used on the library page — instead a local useState<SubtitleSettings> manages state.

   IMPORTANT: The `useSubtitleSettings` hook in `use-subtitle-settings.ts` does NOT need modification — its merge pattern (`{ ...DEFAULT_SUBTITLE_SETTINGS, ...parsed }` at line 28) automatically picks up new optional fields from DEFAULT_SUBTITLE_SETTINGS. No code change needed in the hook.
  </action>
  <verify>
