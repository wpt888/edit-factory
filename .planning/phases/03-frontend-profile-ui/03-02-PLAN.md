...
must_haves:
  ...
  artifacts:
    - path: "frontend/src/components/create-profile-dialog.tsx"
      provides: "Modal dialog for creating new profiles with validation"
...
<task type="auto">
  <name>Task 1: Create CreateProfileDialog component with validation</name>
...
4. handleCreate function with COMPLETE VALIDATION:
   - Trim name before validation
   - Check minimum length: if (name.trim().length < 2) { toast.error('Profile name must be at least 2 characters'); return; }
   - Check maximum length: if (name.trim().length > 50) { toast.error('Profile name must be 50 characters or less'); return; }
   - POST to /profiles with { name: name.trim(), description }
...
5. Form layout:
   ...
   - Show character count hint below input: "{name.length}/50 characters"
...
AVOID: Not resetting form fields after successful creation. AVOID: Not calling refreshProfiles (stale dropdown). AVOID: Only checking for empty string without min/max length validation.
  </action>
  <verify>
File exists and exports CreateProfileDialog. TypeScript compilation passes: `cd frontend && npx tsc --noEmit`. Grep confirms validation patterns: `grep -E "(length\s*<\s*2|length\s*>\s*50|at least 2|50 characters)" frontend/src/components/create-profile-dialog.tsx`
  </verify>
  <done>
CreateProfileDialog component created with complete form validation (min 2 chars, max 50 chars), API call, and profile list refresh.
  </done>
</task>
...
6. Validation logic present: grep confirms min/max length checks in CreateProfileDialog
</verification>
...
- CreateProfileDialog validates profile name length (min 2, max 50 characters)
