...
must_haves:
  truths:
    ...
    - "Blurry segments rejected before scoring (blur_score < 0.2, corresponding to Laplacian variance < 100)"
...
**4. Add two new methods to VideoAnalyzer class (insert between _read_frame_at and _calculate_motion_for_interval, at line ~171):**
...
**6. Update analyze_full_video (starts at line ~228):**
- Find the line where `motion_score, variance_score = self._calculate_motion_for_interval(...)` is unpacked. Change to `motion_score, variance_score, blur_score, contrast_score = ...`
- Add blur rejection threshold after the existing MIN_BRIGHTNESS_THRESHOLD and MIN_MOTION_THRESHOLD:
  ```python
