...
must_haves:
  truths:
    - "User can view their list of profiles via API"
    - "User can create a new profile via API"
    - "User cannot delete their default profile (protected)"
    - "API calls without X-Profile-Id header still work (auto-selects default)"
    - "API calls with wrong profile return clear error (not 500)"
  artifacts:
...
Run: `python -c "from app.api.auth import ProfileContext, get_profile_context; print('OK')"` returns OK
  ...
  <done>
ProfileContext class importable, get_profile_context dependency exists with proper validation logic for missing/invalid/foreign profiles. Missing default profile returns 503 with actionable message.
  </done>
</task>
...
5. **DELETE /profiles/{profile_id}** - Delete profile
   - Depends on get_current_user
   - CANNOT delete if is_default=True (return 400 with message "Cannot delete default profile. Set another profile as default first.")
...
3. **Error message check for missing default:**
   ```bash
   grep -q "Account misconfigured" app/api/auth.py && echo "Actionable error message present"
   ```
...
- Missing default profile returns 503 (not 500) with actionable message mentioning "contact support" or "re-run setup"
- Profile CRUD endpoints exist at /api/v1/profiles/*
- Default profile deletion returns 400 error with helpful message
