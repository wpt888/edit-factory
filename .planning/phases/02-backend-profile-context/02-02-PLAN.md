...
must_haves:
  truths:
    - "JobStorage.create_job accepts profile_id parameter and stores it in job data"
    - "JobStorage.list_jobs can filter by profile_id"
    - "CostTracker.log_elevenlabs_tts accepts profile_id and includes it in Supabase insert"
    - "CostTracker.log_gemini_analysis accepts profile_id and includes it in Supabase insert"
    - "CostTracker.get_summary can filter by profile_id"
    - "PostizPublisher methods log profile_id for debugging"
  artifacts:
...
IMPORTANT: All parameters are Optional with default None to maintain backward compatibility. Existing callers without profile_id will continue working.
  ...
  <verify>
Run: `python -c "from app.services.job_storage import JobStorage; import inspect; sig = inspect.signature(JobStorage.create_job); print('profile_id' in str(sig))"` returns True
  </verify>
  <done>
JobStorage.create_job, update_job, and list_jobs accept profile_id parameter, store/filter it in database, and log with profile context.
  </done>
</task>
...
IMPORTANT: profile_id=None means "all profiles" (backward compatible). When profile_id is provided, filter results to that profile only.
  ...
  <verify>
Run: `python -c "from app.services.cost_tracker import CostTracker; import inspect; sig = inspect.signature(CostTracker.log_elevenlabs_tts); print('profile_id' in str(sig))"` returns True
  </verify>
  <done>
CostTracker.log_elevenlabs_tts, log_gemini_analysis, and get_summary accept profile_id parameter, insert it into database, and filter results accordingly.
  </done>
</task>
...
IMPORTANT: All changes are logging-only for Phase 2. The profile_id parameter is added for traceability but doesn't change Postiz credentials (that's Phase 5).
  ...
  <verify>
Run: `python -c "from app.services.postiz_service import PostizPublisher; import inspect; sig = inspect.signature(PostizPublisher.create_post); print('profile_id' in str(sig))"` returns True
  </verify>
  <done>
PostizPublisher methods accept profile_id parameter and include it in log messages for per-profile debugging and traceability.
  </done>
</task>
...
<success_criteria>
- JobStorage.create_job, update_job, list_jobs accept profile_id parameter
- CostTracker.log_elevenlabs_tts, log_gemini_analysis, get_summary accept profile_id parameter
- PostizPublisher.upload_video, create_post, get_integrations accept profile_id parameter
