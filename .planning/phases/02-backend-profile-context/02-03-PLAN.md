...
must_haves:
  truths:
    - "POST /library/projects includes profile_id in database insert"
    - "GET /library/projects filters by profile_id from header"
    - "All project/clip routes validate profile ownership before operations"
    - "Background task _generate_raw_clips_task receives profile_id parameter"
    - "Background tasks pass profile_id to CostTracker and JobStorage calls"
    - "Logging includes [Profile {id}] prefix in background tasks"
  artifacts:
...
<task type="auto">
  <name>Task 3: Update background tasks to use profile_id</name>
  <files>app/api/library_routes.py</files>
  <action>
Update all background task functions to accept and use profile_id parameter.
...
Pass profile_id to CostTracker calls (if any Gemini analysis in this task):
```python
...
Same pattern - add profile_id parameter and logging.
...
Pass to CostTracker for TTS cost logging:
```python
...
**JobStorage calls in background tasks:**
If background tasks create or update jobs, pass profile_id:
```python
...
```
  ...
  <verify>
Run: `grep -n "def _.*_task\|async def _.*_task" app/api/library_routes.py | head -10` then verify each task signature includes profile_id
  </verify>
  <done>
All background task functions accept profile_id parameter, include it in logging, and pass it to CostTracker/JobStorage calls.
  </done>
</task>
...
4. **No profile-less project queries:**
   ```bash
...
- All CostTracker/JobStorage calls in background tasks pass profile_id
