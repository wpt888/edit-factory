...
5. `POST /feeds/{feed_id}/sync` — Trigger sync as BackgroundTask. Set `sync_status='syncing'` immediately. Background task: download XML with httpx (set User-Agent header per research), call `parse_feed_xml`, call `upsert_products`, update feed status to idle with product_count and last_synced_at. On error: set `sync_status='error'` with error message.
...
app.include_router(feed_router, prefix="/api/v1/feeds", tags=["feeds"])
```
Follow the exact same pattern used for other routers (library_router, segments_router, etc.).
  </action>
  <verify>
